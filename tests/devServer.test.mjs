...
test("ローカルserverはA/B/C/Dと依存JS/CSSを配信しwhitelist外を拒否", { timeout: 20000 }, async t => {
  ...
  const base = `http://127.0.0.1:${port}`, queue = ["/a-skill-test.html", "/b-skill-test.html", "/c-skill-test.html", "/d-skill-test.html"], visited = new Set();
  while (queue.length) {
