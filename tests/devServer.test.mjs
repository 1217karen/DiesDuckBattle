...
test("ローカルserverはsetting・A/B/C/Dと依存JS/CSSを配信しwhitelist外を拒否", { timeout: 20000 }, async t => {
  ...
  const base = `http://127.0.0.1:${port}`, queue = ["/select.html", "/setting.html", "/a-skill-test.html", "/b-skill-test.html", "/c-skill-test.html", "/d-skill-test.html"], visited = new Set();
  while (queue.length) {
  ...
  for (const path of ["/js/select.js", "/js/selectState.js", "/css/select.css", "/js/settingPage.js", "/js/settingState.js", "/js/playerBuildStorage.js", "/css/setting.css"]) assert.ok(visited.has(path), path);
